...
- **Microsoft 365 Teams Integration**: Schema and admin settings for mapping clients to Teams and projects to channels. Includes `client_teams`, `project_channels`, and `teams_folder_templates` tables. Configurable channel folder templates with system defaults (Deliverables, SOW & Contracts, Meeting Notes, Status Reports, Working Documents) and tenant-level overrides. Admin UI in Organization Settings for auto-provision toggle, team template selection, folder template editor with drag-to-reorder, and Graph API connection testing.
